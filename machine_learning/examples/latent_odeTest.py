...
def generate_sine_wave(nsinewaves = 1000,
                       ntotal = 500,
                       nsample = 100,
                       start = 0,
                       stop = 20*np.pi,
    ...
    x, y = np.copy(orig_ts), a * np.sin(b * orig_ts)
    
    orig_traj = np.stack((x, y), axis = 1)
...
#    )
    ...
    orig_trajs, samp_trajs, orig_ts, samp_ts = generate_sine_wave(nsinewaves = nsinewaves,
                       ntotal = ntotal,
                       nsample = nsample,
                       start = start,
                       stop = stop,
    ...
    rec = RecognitionRNN(latent_dim, obs_dim, rnn_nhidden, nsinewaves).to(device)
